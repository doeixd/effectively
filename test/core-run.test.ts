...
describe("Core Execution Engine (run.ts)", () => {
  ...
  describe("Error handling and Result types", () => {
    ...
    it("should handle successful tasks with throw: false", async () => {
      ...
      const successTask = defineTask(async (input: string) =>
        input.toUpperCase(),
      );
  ...
  describe("Backtracking and control flow", () => {
    ...
    it("should prevent infinite backtracking loops", async () => {
      ...
      await expect(run(infiniteRetryTask, 1)).rejects.toThrow(
  ...
  describe("Smart, Local, and Global context functions", () => {
    ...
    it("should throw errors for local-only functions when no context", () => {
      expect(() => defineTaskLocal(() => Promise.resolve("test"))).toThrow(
        ContextNotFoundError,
      );
  ...
  describe("Context validation and merging", () => {
    ...
    it("should require context properties", async () => {
      ...
      const requirePropsTask = defineTask(async () => {
        const ctx = requireContextProperties<TestContext>(
          "userId" as keyof TestContext,
          "database" as keyof TestContext,
        );
        return ctx.userId;
      });
      ...
      await expect(run(missingPropTask, undefined)).rejects.toThrow(
        WorkflowError,
      );
    });
  });
  ...
  describe("Dependency injection", () => {
    it("should create and use injection tokens", async () => {
      const { run, defineTask, provide, inject, getContext } =
        createContext<TestContext>(testContextDefaults);

      const userServiceToken = createInjectionToken<{
    ...
    it("should handle optional injection", async () => {
      const { run, defineTask, injectOptional } =
        createContext<TestContext>(testContextDefaults);

      const optionalServiceToken =
        createInjectionToken<string>("OptionalService");
    ...
    it("should throw when required injection not found", async () => {
      const { run, defineTask, inject } =
        createContext<TestContext>(testContextDefaults);

      const requiredServiceToken =
        createInjectionToken<string>("RequiredService");

      const requiredInjectionTask = defineTask(async () => {
        return inject(requiredServiceToken);
      });

      await expect(run(requiredInjectionTask, undefined)).rejects.toThrow(
        "Injection token",
      );
    });
  });
  ...
  describe("Cancellation and abort signals", () => {
    it("should propagate abort signals to context scope", async () => {
      ...
      const promise = run(signalTask, undefined, {
    ...
    it("should handle abort during execution", async () => {
      ...
      const promise = run(longRunningTask, undefined, {
        parentSignal: abortController.signal,
      });
