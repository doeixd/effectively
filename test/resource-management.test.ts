...
describe("Resource Management (bracket.ts)", () => {
  const { run, defineTask, getContext } =
  ...
  describe("withResource (bracket)", () => {
    ...
    it("should release resources even when use task throws", async () => {
      ...
      await expect(run(workflow, "test-input")).rejects.toThrow(
        "Use task failed",
      );
    ...
    it("should handle errors in both use and release phases", async () => {
      const workflow = withResource({
        ...
        release: defineTask(async () => {
          throw new Error("Release failed");
        }),
        merge: mergeIntoContext("db"),
      });

      const promise = run(workflow, "test-input");
      await expect(promise).rejects.toThrow(WorkflowError);

      try {
        await promise;
      } catch (e: any) {
        expect(e.cause).toBeInstanceOf(AggregateError);
        const aggregateError = e.cause as AggregateError;
        expect(aggregateError.errors[0].message).toBe("Use failed");
        expect(aggregateError.errors[1].message).toBe("Release failed");
      }
    });
    ...
    it("should work with complex resource merging", async () => {
      ...
      type MergedContext = TestContext & {
        db: MockConnection;
        cache: MockConnection;
      };
      const useTask = defineTask(async (input: string) => {
      ...
      const releaseTask = defineTask(
        async (resources: { db: MockConnection; cache: MockConnection }) => {
  ...
  describe("Disposable and helper functions", () => {
    ...
    it("createResource should create a valid resource definition", async () => {
      ...
      const merged = resourceDef.merge(
        { scope: { signal: new AbortController().signal }, logs: [] },
        new MockConnection("merged"),
      );
  ...
  describe("withDisposableResource", () => {
    it("should work with synchronous disposable resources", async () => {
      ...
      const workflow = withDisposableResource({
        ...
        use: defineTask(async () => {
          const ctx = getContext() as TestContext & {
            resource: MockSyncDisposableResource;
          };
  ...
  describe("withResources (bracketMany)", () => {
    it("should manage multiple resources correctly", async () => {
      ...
      const dbResource = createResource(
        ...
        defineTask(async () => {
          order.push("acq db");
          return new MockConnection("db");
        }),
        defineTask(async (resource: MockConnection) => {
          order.push("rel db");
      ...
      const cacheResource = createResource(
        "cache",
        defineTask(async () => {
          order.push("acq cache");
          return new MockConnection("cache");
        }),
        defineTask(async (resource: MockConnection) => {
          order.push("rel cache");
          await resource.close();
        }),
      );
    ...
    it("should release acquired resources if a later acquisition fails", async () => {
      ...
      const dbResource = createResource(
        ...
        defineTask(async (_resource: MockConnection) => {
          order.push("rel db");
        }),
      );
      const failingResource = createResource(
        ...
        defineTask(async () => {
          order.push("acq cache");
          throw new Error("Cache acquire failed");
        }),
        defineTask(async (_resource: MockConnection) => {
          order.push("rel cache");
        }),
