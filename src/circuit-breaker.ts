...
export function withCircuitBreaker<C extends { scope: Scope; logger?: Logger }, V, R>(
  ...
  const circuitBreakerEnhancedTask: Task<C, V, R> = async (context: C, value: V): Promise<R> => {
    ...
    if (state.status === 'OPEN') {
      ...
      if (timeSinceFailure > openStateTimeoutMs && !state.isTrialRequestInProgress) {
    ...
    try {
      ...
      if (state.status === 'HALF-OPEN' && state.isTrialRequestInProgress && !isFailure(error)) {
        state.isTrialRequestInProgress = false;
      ...
      logger.warn(`[Circuit Breaker: ${id}] Recorded failure #${state.failures}. Last failure at ${state.lastFailureTimestamp}`, { error });
      ...
      } else if (state.status === 'CLOSED' && state.failures >= failureThreshold) {
