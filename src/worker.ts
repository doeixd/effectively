...
export function createWorkerHandler(
  ...
  // Note: This eval is required by seroval for cross-reference support in workers
  // It only executes trusted code generated by seroval's getCrossReferenceHeader
  ...
  return async (fullContext: C, value: V): Promise<AsyncIterable<R>> => {
    ...
    return (async function* () {
      ...
      worker.onmessage = (e: MessageEvent<string>) => {
        ...
        // Note: This eval executes serialized commands from seroval for streaming support
        // It only processes trusted data generated by seroval's streaming protocol
        try { 
          // eslint-disable-next-line no-eval
          eval(e.data); 
        } catch (err) { 
          streamProxy.throw(err); 
        }
      };

      try {
        // Manually implement async iteration for the stream proxy
        let done = false;
        const buffer: R[] = [];
        const pending: Array<{ resolve: (value: IteratorResult<R>) => void, reject: (error: any) => void }> = [];

        const cleanup2 = streamProxy.on({
          next(value: R) {
            if (pending.length > 0) {
        ...
        while (!done) {
          if (buffer.length > 0) {
            ...
            await new Promise<IteratorResult<R>>((resolve, reject) => {
